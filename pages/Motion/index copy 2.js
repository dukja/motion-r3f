...
const ANIMATION_SETTINGS = {
  ...
  MEET_START: 0.4,
  MEET_END: 0.6,
  TOTAL_DURATION: 5, // 총 애니메이션 지속 시간 (초)
  ...
  get MEET_POINT_Y() {
    return window.innerHeight * 0.5; // 화면 중앙
  },
  get MEET_POINT_THREE_X() {
    return (this.MEET_POINT - 0.5) * 5; // Three.js 좌표계로 변환
  },
...
export const SynchronizedAnimation = () => {
  ...
  useEffect(() => {
    ...
    const lastSection = sectionRefs[sectionRefs.length - 1]; // Section 5

    // ThreeDElement와 CurrentDomElement 애니메이션 (모든 섹션에서 지속)
    gsap.to(threeDRef.current.position, {
      x: gsap.utils.wrap(-2.5, 2.5), // 스크롤 방향으로 이동
      repeat: -1,
      duration: 20,
      ease: "none",
      scrollTrigger: {
    ...
    // CurrentDomElement 애니메이션 (ThreeDElement와 동일하게 움직임)
    ...
    const totalDuration = ANIMATION_SETTINGS.TOTAL_DURATION;
    const meetPoint = ANIMATION_SETTINGS.MEET_POINT * window.innerWidth;
    const meetStart = ANIMATION_SETTINGS.MEET_START * totalDuration;
    const meetEnd = ANIMATION_SETTINGS.MEET_END * totalDuration;

    tl.fromTo(
      domRef.current,
      { x: window.innerWidth },
      { x: meetPoint, duration: meetStart, ease: "power2.inOut" }
    )
      .to([threeDRef.current.position, currentDomRef.current, domRef.current], {
        x: gsap.utils.wrap([
          ANIMATION_SETTINGS.MEET_POINT_THREE_X,
          meetPoint,
          meetPoint,
        ]),
        duration: meetEnd - meetStart,
      })
      .to(
        threeDRef.current.position,
        { x: 2.5, duration: totalDuration - meetEnd, ease: "power2.in" },
        "split"
      )
      .to(
        currentDomRef.current,
        {
          x: window.innerWidth,
          duration: totalDuration - meetEnd,
          ease: "power2.in",
        },
        "split"
      )
      .to(
        domRef.current,
        { x: -50, duration: totalDuration - meetEnd, ease: "power2.in" },
        "split"
      );
